...
from bedRMod import write_header_from_dict, write_data_from_df
...
def test_write_header():
    ...
    write_header_from_dict(f"{test_dir}/test_write2bedrmod.bedrmod", header_dict)

    assert header_dict == read_header(f"{test_dir}/test_write2bedrmod.bedrmod")

def test_write_data():
    data_df = read_data(f"{test_dir}/test_df2bedrmod.bedrmod")
    write_data_from_df(f"{test_dir}/test_write2bedrmod.bedrmod", data_df)
